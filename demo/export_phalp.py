...
https://github.com/vye16/slahmr/blob/main/slahmr/preproc/export_phalp.py
"""
...
def unpack_frame(track_data):
    ...
    for i, tid in enumerate(track_data["tid"]):
        ...
        # get perspective camera translation.
        bs = scale * cam[0] + 1e-6
        tz = 2 * focal / bs
        tx = cam[1] + (2 * (cx - W / 2) / bs)
        ty = cam[2] + (2 * (cy - H / 2) / bs)
