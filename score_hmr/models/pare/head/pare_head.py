...
from score_hmr.utils.geometry import rot6d_to_rotmat
...
class PareHead(nn.Module):
    def __init__(
        ...
        if backbone.startswith("hrnet"):
            ...
            conv_fn = (
                self._make_upsample_layer if use_upsampling else self._make_deconv_layer
            )
        ...
        if self.use_soft_attention:
            ...
            self.keypoint_final_layer = nn.Sequential(
                ...
                conv1x1(
                    ...
                    (
                        num_joints + 1
                        if self.use_heatmaps in ("part_segm", "part_segm_pool")
                        else num_joints
                    ),
                ),
            )

            soft_att_feature_size = (
                smpl_final_dim  # if use_hmr_regression else pose_mlp_inp_dim
            )
        ...
        if self.use_hmr_regression:
            ...
            self.fc1 = nn.Linear(
                num_joints * smpl_final_dim + (num_joints * 6) + 10 + num_camera_params,
                1024,
            )
    ...
    def _prepare_pose_mlp_inp(self, feats, pred_pose, pred_shape, pred_cam):
        ...
        for inp_type in self.pose_input_type:
            ...
            if inp_type == "all_pose":
                ...
                all_pose = pred_pose.reshape(batch_size, -1, 1)[..., None].repeat(
                    1, 1, num_joints, 1
                )
    ...
    def forward(self, features, gt_segm=None):
        ...
        if gt_segm is not None:
            ...
            part_attention = (
                F.one_hot(gt_segm.to("cpu"), num_classes=self.num_joints + 1)
                .permute(0, 3, 1, 2)
                .float()[:, 1:, :, :]
            )
    ...
    def _get_final_preds(
        self, pose_feats, cam_shape_feats, init_pose, init_shape, init_cam
    ):
        if self.use_hmr_regression:
            return self._hmr_get_final_preds(
                cam_shape_feats, init_pose, init_shape, init_cam
            )
        else:
    ...
    def _pare_get_final_preds(
        ...
        if self.iterative_regression:
            ...
            if self.use_final_nonlocal:
                pose_feats = self.final_pose_nonlocal(pose_feats.squeeze(-1)).unsqueeze(
                    -1
                )
