...
def softargmax1d(
    ...
    points = (
        torch.arange(0, dim, device=device, dtype=dtype)
        .reshape(1, 1, dim)
...
def softargmax2d(
    ...
    x = (
        ...
        .reshape(1, 1, 1, width)
        .expand(batch_size, -1, height, -1)
    )
    y = (
        torch.arange(0, height, device=device, dtype=dtype)
        .reshape(1, 1, height, 1)
        .expand(batch_size, -1, -1, width)
    )
    # Should be Bx2xHxW
    points = torch.cat([x, y], dim=1)
    normalized_heatmap = _softmax(
...
def softargmax3d(
    ...
    x = (
        torch.arange(0, width, device=device, dtype=dtype)
        .reshape(1, 1, 1, width, 1)
        .expand(batch_size, -1, height, -1, depth)
    )
    y = (
        torch.arange(0, height, device=device, dtype=dtype)
        .reshape(1, 1, height, 1, 1)
        .expand(batch_size, -1, -1, width, depth)
    )
    z = (
        torch.arange(0, depth, device=device, dtype=dtype)
        .reshape(1, 1, 1, 1, depth)
        .expand(batch_size, -1, height, width, -1)
    )
    # Should be Bx2xHxW
