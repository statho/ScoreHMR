"""
Skeleton Renderer, which is only used during training for tensorboard logging.
Code was borrowed from: https://github.com/nkolot/ProHMR/blob/master/prohmr/utils/skeleton_renderer.py
"""
import torch
import numpy as np
import trimesh
...
class SkeletonRenderer:
    ...
    def __call__(
        ...
        gt_keypoints_3d[:, :, :-1] = (
            gt_keypoints_3d[:, :, :-1]
            - gt_keypoints_3d[:, [25 + 14], :-1]
            + pred_keypoints_3d[:, 0, [25 + 14]]
        )
        gt_keypoints_2d = gt_keypoints_2d.clone().cpu().float().numpy()
        gt_keypoints_2d[:, :, :-1] = (
            self.cfg.MODEL.IMAGE_SIZE * (gt_keypoints_2d[:, :, :-1] + 1.0) / 2.0
        )
                ...
                torch.tensor(
                    [
                        ...
                        * self.cfg.EXTRA.FOCAL_LENGTH
                        / (0.8 * self.cfg.MODEL.IMAGE_SIZE),
                    ]
                )
                .unsqueeze(0)
                .unsqueeze(0)
                .repeat(batch_size, num_samples, 1)
            )
        else:
        ...
        if images is None:
            images = np.zeros(
                (batch_size, self.cfg.MODEL.IMAGE_SIZE, self.cfg.MODEL.IMAGE_SIZE, 3)
            )
        ...
        camera_center = (
            torch.tensor(
                [self.cfg.MODEL.IMAGE_SIZE, self.cfg.MODEL.IMAGE_SIZE],
                dtype=torch.float,
            ).reshape(1, 2)
            / 2.0
        )
        gt_keypoints_3d_proj = perspective_projection(
            gt_keypoints_3d[:, :, :-1],
            rotation=rotation.repeat(batch_size, 1, 1),
            translation=camera_translation[:, 0, :],
            focal_length=focal_length.repeat(batch_size, 1),
            camera_center=camera_center.repeat(batch_size, 1),
        )
        pred_keypoints_3d_proj = perspective_projection(
            pred_keypoints_3d.reshape(batch_size * num_samples, -1, 3),
            rotation=rotation.repeat(batch_size * num_samples, 1, 1),
            translation=camera_translation.reshape(batch_size * num_samples, -1),
            focal_length=focal_length.repeat(batch_size * num_samples, 1),
            camera_center=camera_center.repeat(batch_size * num_samples, 1),
        ).reshape(batch_size, num_samples, -1, 2)
        gt_keypoints_3d_proj = (
            torch.cat([gt_keypoints_3d_proj, gt_keypoints_3d[:, :, [-1]]], dim=-1)
            .cpu()
            .numpy()
        )
        pred_keypoints_3d_proj = (
            torch.cat(
                [
                    pred_keypoints_3d_proj,
                    keypoints_to_render.reshape(batch_size, 1, -1, 1).repeat(
                        1, num_samples, 1, 1
                    ),
                ],
                dim=-1,
        ...
        R = torch.tensor(
            trimesh.transformations.rotation_matrix(np.radians(90), [0, 1, 0])[:3, :3]
        ).float()
        gt_keypoints_3d_side = gt_keypoints_3d.clone()
        gt_keypoints_3d_side[:, :, :-1] = torch.einsum(
            "bni,ij->bnj", gt_keypoints_3d_side[:, :, :-1], R
        )
        pred_keypoints_3d_side = pred_keypoints_3d.clone()
        pred_keypoints_3d_side = torch.einsum(
            "bmni,ij->bmnj", pred_keypoints_3d_side, R
        )
        gt_keypoints_3d_proj_side = perspective_projection(
            gt_keypoints_3d_side[:, :, :-1],
            rotation=rotation.repeat(batch_size, 1, 1),
            translation=camera_translation[:, 0, :],
            focal_length=focal_length.repeat(batch_size, 1),
            camera_center=camera_center.repeat(batch_size, 1),
        )
        pred_keypoints_3d_proj_side = perspective_projection(
            pred_keypoints_3d_side.reshape(batch_size * num_samples, -1, 3),
            rotation=rotation.repeat(batch_size * num_samples, 1, 1),
            translation=camera_translation.reshape(batch_size * num_samples, -1),
            focal_length=focal_length.repeat(batch_size * num_samples, 1),
            camera_center=camera_center.repeat(batch_size * num_samples, 1),
        ).reshape(batch_size, num_samples, -1, 2)
        gt_keypoints_3d_proj_side = (
            torch.cat(
                [gt_keypoints_3d_proj_side, gt_keypoints_3d_side[:, :, [-1]]], dim=-1
            )
            .cpu()
            .numpy()
        )
        pred_keypoints_3d_proj_side = (
            torch.cat(
                [
                    pred_keypoints_3d_proj_side,
                    keypoints_to_render.reshape(batch_size, 1, -1, 1).repeat(
        ...
        for i in range(batch_size):
            img = images[i]
            side_img = np.zeros(
                (self.cfg.MODEL.IMAGE_SIZE, self.cfg.MODEL.IMAGE_SIZE, 3)
            )
            # gt 2D keypoints
            body_keypoints_2d = gt_keypoints_2d[i, :25].copy()
            # print('Rendering OpenPose KP')
            ...
            gt_keypoints_img = render_openpose(img, body_keypoints_2d) / 255.0
            # gt 3D keypoints
            ...
            gt_keypoints_3d_proj_img = (
                render_openpose(img, body_keypoints_3d_proj) / 255.0
            )
            # gt 3D keypoints from the side
            body_keypoints_3d_proj = gt_keypoints_3d_proj_side[i, :25].copy()
            for op, gt in zip(openpose_indices, gt_indices):
                if (
                    gt_keypoints_3d_proj_side[i, gt, -1]
                    > body_keypoints_3d_proj[op, -1]
                ):
                    body_keypoints_3d_proj[op] = gt_keypoints_3d_proj_side[i, gt]
            gt_keypoints_3d_proj_img_side = (
                render_openpose(side_img, body_keypoints_3d_proj) / 255.0
            )
            # pred 3D keypoints
            pred_keypoints_3d_proj_imgs = []
            for j in range(num_samples):
                body_keypoints_3d_proj = pred_keypoints_3d_proj[i, j, :25].copy()
                for op, gt in zip(openpose_indices, gt_indices):
                    if (
                        pred_keypoints_3d_proj[i, j, gt, -1]
                        >= body_keypoints_3d_proj[op, -1]
                    ):
                        body_keypoints_3d_proj[op] = pred_keypoints_3d_proj[i, j, gt]
                pred_keypoints_3d_proj_imgs.append(
                    render_openpose(img, body_keypoints_3d_proj) / 255.0
                )
            pred_keypoints_3d_proj_img = np.concatenate(
                pred_keypoints_3d_proj_imgs, axis=1
            )
            # gt 3D keypoints from the side
            pred_keypoints_3d_proj_imgs_side = []
            for j in range(num_samples):
                body_keypoints_3d_proj = pred_keypoints_3d_proj_side[i, j, :25].copy()
                for op, gt in zip(openpose_indices, gt_indices):
                    if (
                        pred_keypoints_3d_proj_side[i, j, gt, -1]
                        >= body_keypoints_3d_proj[op, -1]
                    ):
                        body_keypoints_3d_proj[op] = pred_keypoints_3d_proj_side[
                            i, j, gt
                        ]
                pred_keypoints_3d_proj_imgs_side.append(
                    render_openpose(side_img, body_keypoints_3d_proj) / 255.0
                )
            pred_keypoints_3d_proj_img_side = np.concatenate(
                pred_keypoints_3d_proj_imgs_side, axis=1
            )
            rows.append(
                np.concatenate(
                    (
                        gt_keypoints_img,
                        gt_keypoints_3d_proj_img,
                        pred_keypoints_3d_proj_img,
                        gt_keypoints_3d_proj_img_side,
                        pred_keypoints_3d_proj_img_side,
                    ),
                    axis=1,
                )
            )
        ...
        img[:: self.cfg.MODEL.IMAGE_SIZE, :, :] = 1.0
        img[:, (1 + 1 + num_samples) * self.cfg.MODEL.IMAGE_SIZE, :] = 0.5
        return img
